...
function clauses(message) {
	...
	if (lowmessage.includes("standard")) { message.channel.send("Standard Clauses: Shorthand for Sleep, Freeze, OHKO, Accuracy, Evasion, Imprison Clauses as well as Dynamax, Z-Moves, Megas, and Power Construct disallowed and default weather and terrain.  For more information on any of theses, see the appropriate `,clause` help (and you can include multiple in the same command)."); }
...
bot.on("messageUpdate", async function(oldMessage, newMessage) {
    ...
    if (oldMessage.author.bot) {
        if (oldMessage.author.id == bot.user.id && oldMessage.content != newMessage.content) {
